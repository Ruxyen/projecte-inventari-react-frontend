...
export default function NewEditDialog({ open, setOpen, currentItem, setCurrentItem, getList }) {
  ...
  return (
    ...
    >
      {/* Título del diálogo */}
      <DialogTitle>{currentItem._id === null ? "Nueva Incidencia" : "Editar Incidencia"}</DialogTitle>
      <DialogContent>
        {/* Texto descriptivo */}
        <DialogContentText>
          {currentItem._id === null ? "Completa los datos para crear una nueva incidencia." : "Modifica los datos de la incidencia."}
        </DialogContentText>
        ...
        {/* Campos de formulario para prioridad, descripción y tipo */}
        <TextField margin="dense" id="priority" name="priority" label="Prioridad" select fullWidth variant="outlined"
          value={currentItem.priority} onChange={handleChange}>
          <MenuItem value="Alta">Alta</MenuItem>
          <MenuItem value="Media">Media</MenuItem>
          <MenuItem value="Baja">Baja</MenuItem>
        </TextField>

        <TextField margin="dense" id="description" name="description" label="Descripción" fullWidth multiline rows={4} variant="outlined"
          value={currentItem.description} onChange={handleChange} />

        <TextField margin="dense" id="type" name="type" label="Tipo" select fullWidth variant="outlined"
          value={currentItem.type} onChange={handleChange}>
